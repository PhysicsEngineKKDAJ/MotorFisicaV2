...
{
	ang = world->getNumRandom(0, 360);
	Particles *p = new Particles(PuntoVector3D(world->getNumRandom(55, -55) * cos(ang), 180, world->getNumRandom(55, -55) * sin (ang), 0), -0.8, 200);
	//Cambiar los colores con switch
	int randomColor = world->getNumRandom(4, 0);
	switch (randomColor)
	{
