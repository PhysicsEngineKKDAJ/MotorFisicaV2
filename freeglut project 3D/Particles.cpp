...
void Particles::update(GLfloat dt) {
	...
	PuntoVector3D *aux = new PuntoVector3D(dt, dt, dt, 1);
	aceleration->productoEscalar(aux);
	vel->sumar(aceleration);

	PuntoVector3D *Velocidad= vel->clonar();
	Velocidad->productoEscalar(aux);
	delete aux;

	pos->sumar(Velocidad);
	aux = new PuntoVector3D(0, 0, 0, 1);
	acc->productoEscalar(aux);
	delete aux;
